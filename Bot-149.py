    ...
    if user_id==ADMIN_ID:
        ...
        if text == "/toggleotc":
            ...
            if new_state:
                await update.message.reply_text(
                    "✅ *OTC Pairs: WASHA (ON)*\n\n"
                    "Pairs zote zinaonekana sasa — OTC na non-OTC.\n\n"
                    "_Tumia /toggleotc tena kuzima OTC._",
                    parse_mode="Markdown"
                )
            else:
                await update.message.reply_text(
                    "🔴 *OTC Pairs: ZIMA (OFF)*\n\n"
                    "Watumie wataona *non-OTC pairs tu* sasa.\n"
...
async def run_bot():
    ...
    if RENDER_URL:
        # ── WEBHOOK MODE (Render na URL imewekwa) ──────────────
        print("Webhook mode — port {} — {}".format(PORT, RENDER_URL))
        ...
        print("Webhook set: {}".format(WEBHOOK_URL))
        ...
        async def telegram_webhook(request):
            try:
                ...
                logging.warning("Webhook process error: {}".format(e))
        ...
        # ── POLLING MODE na dummy HTTP server ─────────────────
        # Fungua port kwanza (thread ya pili) ili Render isimame
        import threading
        ...
        def start_health_server():
            ...
            print("Health server on port {}".format(PORT))
