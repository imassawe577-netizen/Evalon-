...
async def schedule_result_check(bot, chat_id, user_id, pair, direction, timeframe_mins, entry_price):
    ...
    # FIX: Compare direction shown to user vs actual price movement
    # direction is what was shown to user (post-reverse if applicable)
    ...
    if direction == "BUY":
        won = price_diff > 0   # Price went up = user wins
    else:
        won = price_diff < 0   # Price went down = user wins
...
VIRTUAL_TF_SECONDS = [60, 120, 180]  # 1m, 2m, 3m in seconds
...
async def _vt_check_results():
    """
    Check all pending virtual trades whose expiry has passed.
    Fetch exit price, determine win/loss, update pair_stats.
    """
    now = time.time()
    pairs_checked = set()
    for key in list(_virtual_trades.keys()):
        ...
        for (entry_price, direction, expiry) in _virtual_trades[key]:
            ...
            # Update stats — virtual trades don't trigger reverse logic
            try:
                with get_conn() as conn:
                    with conn.cursor() as cur:
                        if won:
    ...
    if pairs_checked:
        logging.info("VTE: checked results for {} pairs".format(len(pairs_checked)))
